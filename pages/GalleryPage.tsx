  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
        <div>
            <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Galeria de Mídia</h2>
            <p className="text-gray-500 mt-1">Gerencie e compartilhe os arquivos da igreja.</p>
        </div>
        <button 
          onClick={handleUpload}
          className="bg-accent-500 hover:bg-accent-600 text-white px-6 py-3 rounded-xl shadow-lg shadow-accent-500/20 flex items-center gap-2 transition-all active:scale-95 whitespace-nowrap font-medium"
        >
          <Upload size={18} />
          Upload Mídia
        </button>
          ...
          {['all', 'image', 'video'].map((type) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-sm font-medium capitalize transition-all whitespace-nowrap ${
                filter === type 
                  ? 'bg-church-800 text-white shadow-md' 
                  : 'text-gray-500 hover:bg-gray-50 hover:text-church-600'
              }`}
            >
